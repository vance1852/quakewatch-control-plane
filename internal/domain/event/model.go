...
func (c Candidate) AuthorizeDecision(actorID string, now time.Time, decision Decision, notes string) (DecisionAuthorization, error) {
	if err := c.CanDecide(actorID, now, decision, notes); err != nil {
		return DecisionAuthorization{}, err
	}
	return DecisionAuthorization{
		EventID: c.ID, AnalystID: actorID, Decision: decision,
		Notes: notes, EventVersion: c.Version,
	}, nil
}
