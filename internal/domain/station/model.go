...
// ConflictsWith reports whether an acquisition interval occupies any part of
// the maintenance window. Intervals use half-open bounds so an acquisition
// ending exactly when maintenance starts is allowed.
func (w MaintenanceWindow) ConflictsWith(startsAt, endsAt time.Time) bool {
	startsAt = startsAt.UTC()
	endsAt = endsAt.UTC()
	return startsAt.Before(w.Until) && endsAt.After(w.From)
}
