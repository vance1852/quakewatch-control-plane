...
type SensorReadiness struct {
	Enabled           int
	LatestCalibration time.Time
}

type SensorReadinessCache struct {
	mu    sync.RWMutex
	items map[string]SensorReadiness
}

func NewSensorReadinessCache() *SensorReadinessCache {
	return &SensorReadinessCache{items: make(map[string]SensorReadiness)}
}

func (c *SensorReadinessCache) Put(stationID string, readiness SensorReadiness) {
	c.mu.Lock()
	c.items[stationID] = readiness
	c.mu.Unlock()
}

func (c *SensorReadinessCache) Get(stationID string) (SensorReadiness, bool) {
	c.mu.RLock()
	readiness, ok := c.items[stationID]
	c.mu.RUnlock()
	return readiness, ok
}
