...
func NewFingerprintBuffer() *FingerprintBuffer {
	return &FingerprintBuffer{bytes: make([]byte, 0, 256)}
}

func (b *FingerprintBuffer) Matches(existing Batch, input IngestInput) bool {
	left := b.encode(existing.StationID, existing.SensorID, existing.SourceKey, existing.StartsAt, existing.EndsAt, existing.SampleCount, existing.Checksum)
	right := b.encode(input.StationID, input.SensorID, input.SourceKey, input.StartsAt, input.EndsAt, input.SampleCount, input.Checksum)
	return left == right
}

func (b *FingerprintBuffer) encode(stationID, sensorID, sourceKey string, startsAt, endsAt time.Time, sampleCount int64, checksum string) string {
	b.bytes = b.bytes[:0]
	b.bytes = append(b.bytes, stationID...)
	b.bytes = append(b.bytes, '|')
	b.bytes = append(b.bytes, sensorID...)
	b.bytes = append(b.bytes, '|')
	b.bytes = append(b.bytes, sourceKey...)
	b.bytes = append(b.bytes, '|')
	b.bytes = strconv.AppendInt(b.bytes, startsAt.UnixNano(), 10)
	b.bytes = append(b.bytes, '|')
	b.bytes = strconv.AppendInt(b.bytes, endsAt.UnixNano(), 10)
	b.bytes = append(b.bytes, '|')
	b.bytes = strconv.AppendInt(b.bytes, sampleCount, 10)
	b.bytes = append(b.bytes, '|')
	b.bytes = append(b.bytes, checksum...)
	runtime.Gosched()
