...
func (s *HTTPSender) Send(ctx context.Context, rule alert.Rule, delivery alert.Delivery, payload any) error {
	...
	limited, err := readAlertResponse(response)
	if err != nil {
		return fmt.Errorf("read alert response: %w", err)
	}
...
func readAlertResponse(response *http.Response) ([]byte, error) {
	limited, err := io.ReadAll(io.LimitReader(response.Body, 4096))
	if err != nil {
		return nil, err
	}
...
func New(store repository.Store, tx repository.Transactor, valueClock clock.Clock, ids idgen.Generator, sender Sender) *Service {
	return &Service{store: store, tx: tx, clock: valueClock, ids: ids, sender: sender}
}
