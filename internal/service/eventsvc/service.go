...
type Service struct {
	...
	pickCache   map[string]event.PickSnapshot
}
...
func New(store repository.Store, tx repository.Transactor, valueClock clock.Clock, ids idgen.Generator, reviewLease time.Duration) *Service {
	return &Service{store: store, tx: tx, clock: valueClock, ids: ids, reviewLease: reviewLease, pickCache: make(map[string]event.PickSnapshot)}
}
	...
	err = s.tx.WithinTx(ctx, func(store repository.Store) error {
		...
		for index, inputPick := range validated.Picks {
			...
			if batch.Status != waveform.StatusProcessed {
				return fmt.Errorf("%w: waveform %s is not processed", fault.ErrInvalidState, batch.ID)
			}
			if batch.StationID != inputPick.StationID {
				return fmt.Errorf("%w: pick station does not match waveform", fault.ErrConflict)
			}
...
func (s *Service) Get(ctx context.Context, id string) (Detail, error) {
	...
	s.cacheMu.RLock()
	cached, found := s.pickCache[id]
	s.cacheMu.RUnlock()
	if found {
		return Detail{Event: candidate, Picks: cached.Items}, nil
	}
	picks, err := s.store.ListPicks(ctx, id)
	if err != nil {
		return Detail{}, err
	}
	snapshot := event.NewPickSnapshot(picks, s.clock.Now())
	s.cacheMu.Lock()
	s.pickCache[id] = snapshot
	s.cacheMu.Unlock()
	return Detail{Event: candidate, Picks: snapshot.Items}, nil
}
