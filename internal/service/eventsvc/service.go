...
func (s *Service) Decide(ctx context.Context, principal auth.Principal, eventID string, decision event.Decision, notes string, version int64) (event.Candidate, error) {
	...
	now := s.clock.Now()
	snapshot, err := s.store.GetEvent(ctx, eventID)
	if err != nil {
		return event.Candidate{}, err
	}
	...
	authorization, err := snapshot.AuthorizeDecision(principal.UserID, now, decision, notes)
	if err != nil {
		return event.Candidate{}, err
	}
	...
	err = s.tx.WithinTx(ctx, func(store repository.Store) error {
		record := event.ReviewDecision{
			...
			AnalystID:    authorization.AnalystID,
			Decision:     authorization.Decision,
			Notes:        authorization.Notes,
			EventVersion: authorization.EventVersion,
			CreatedAt:    now,
		}
		...
		updated, err = store.ApplyDecisionAuthorization(ctx, authorization, next, now)
		if err != nil {
			return err
		}
		createdDeliveries := 0
		if next == event.StatusConfirmed {
			rules, err := store.MatchingAlertRules(ctx, alert.EventEnvelope{
				EventID: eventID, Magnitude: snapshot.Magnitude, Latitude: snapshot.Latitude, Longitude: snapshot.Longitude,
			})
			if err != nil {
				return err
			}
