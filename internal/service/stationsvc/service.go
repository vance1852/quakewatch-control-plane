...
func (s *Service) ScheduleMaintenance(ctx context.Context, principal auth.Principal, stationID string, version int64, from, until time.Time) (station.Station, error) {
	...
	err = s.tx.WithinTx(ctx, func(store repository.Store) error {
		...
		writeCtx := station.MaintenanceWriteContext(ctx)
		updated, err = store.UpdateStationState(writeCtx, stationID, station.StatusMaintenance, &window.From, &window.Until, version, now)
		if err != nil {
			return err
		}
		return shared.Audit(writeCtx, store, s.ids, &principal.UserID, "station.maintenance_started", "station", stationID, "success", map[string]any{
			"from":  window.From,
			"until": window.Until,
		}, now)
	})
