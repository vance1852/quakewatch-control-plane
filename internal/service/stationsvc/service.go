...
type Service struct {
	...
	clock     clock.Clock
	ids       idgen.Generator
	readiness *station.SensorReadinessCache
}
...
func New(store repository.Store, tx repository.Transactor, valueClock clock.Clock, ids idgen.Generator) *Service {
	return &Service{
		store: store, tx: tx, clock: valueClock, ids: ids,
		readiness: station.NewSensorReadinessCache(),
	}
}
...
func (s *Service) Register(ctx context.Context, principal auth.Principal, input station.RegisterInput) (StationDetail, error) {
	...
	err = s.tx.WithinTx(ctx, func(store repository.Store) error {
		if err := store.CreateStation(ctx, stationValue); err != nil {
			return err
		}
...
func (s *Service) Get(ctx context.Context, id string) (StationDetail, error) {
	...
	readiness := station.SensorReadiness{}
	for _, sensorValue := range sensors {
		if sensorValue.DisabledAt != nil {
			continue
		}
		readiness.Enabled++
		if sensorValue.CalibratedAt.After(readiness.LatestCalibration) {
			readiness.LatestCalibration = sensorValue.CalibratedAt
		}
	}
	s.readiness.Put(id, readiness)
...
func (s *Service) Activate(ctx context.Context, principal auth.Principal, stationID string, version int64) (station.Station, error) {
	...
	err := s.tx.WithinTx(ctx, func(store repository.Store) error {
		...
		readiness, ok := s.readiness.Get(stationID)
		if !ok {
			count, latest, err := store.CountEnabledSensors(ctx, stationID)
			if err != nil {
				return err
			}
			readiness = station.SensorReadiness{Enabled: count, LatestCalibration: latest}
		}
		if err := current.CanTransition(station.StatusActive, readiness.Enabled, readiness.LatestCalibration, now); err != nil {
			return err
		}
		...
		return shared.Audit(ctx, store, s.ids, &principal.UserID, "station.activated", "station", stationID, "success", map[string]any{
			...
			"sensor_count":    readiness.Enabled,
		}, now)
	})
