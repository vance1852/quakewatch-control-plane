...
type Service struct {
	...
	clock        clock.Clock
	ids          idgen.Generator
	fingerprints *waveform.FingerprintBuffer
}
...
func New(store repository.Store, tx repository.Transactor, valueClock clock.Clock, ids idgen.Generator) *Service {
	return &Service{
		...
		fingerprints: waveform.NewFingerprintBuffer(),
	}
}
...
func (s *Service) Ingest(ctx context.Context, principal auth.Principal, input waveform.IngestInput) (IngestResult, error) {
	...
	if err == nil {
		if s.sameWaveform(existing, validated) {
...
func (s *Service) sameWaveform(existing waveform.Batch, input waveform.IngestInput) bool {
	return s.fingerprints.Matches(existing, input)
}
