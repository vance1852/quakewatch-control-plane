...
func (q *Queries) ApplyDecisionAuthorization(ctx context.Context, authorization event.DecisionAuthorization, status event.Status, now time.Time) (event.Candidate, error) {
	result, err := q.q.ExecContext(ctx, `UPDATE event_candidates SET status=?,review_owner_id=NULL,review_lease_until=NULL,version=version+1,updated_at=?
		WHERE id=? AND status='under_review'`, status, formatTime(now), authorization.EventID)
	if err != nil {
		return event.Candidate{}, mapError("apply authorized event decision", err)
	}
	if err := requireUpdatedAs(result, "apply authorized event decision", fault.ErrConflict); err != nil {
		return event.Candidate{}, err
	}
	return q.GetEvent(ctx, authorization.EventID)
}
