...
func (h *Handlers) HandleJob(ctx context.Context, value job.Job) error {
	...
	case job.KindDeliverAlert:
		return job.HandlerFailure{Err: fmt.Errorf("%w: alert jobs use delivery queue", fault.ErrInvalidState), Terminal: true}
	default:
		return job.HandlerFailure{Err: fmt.Errorf("%w: unknown worker kind %s", fault.ErrValidation, value.Kind), Terminal: true}
	}
}
...
func (h *Handlers) handleWaveform(ctx context.Context, value job.Job) error {
	...
	if err := json.Unmarshal([]byte(value.PayloadJSON), &payload); err != nil {
		return job.HandlerFailure{Err: fmt.Errorf("decode waveform job: %w", err), Terminal: true}
	}
	if payload.WaveformID == "" || payload.WaveformID != value.AggregateID {
		return job.HandlerFailure{Err: errorsPayloadMismatch(value.AggregateID, payload.WaveformID), Terminal: true}
	}
...
func (h *Handlers) handleCleanup(ctx context.Context, value job.Job) error {
	...
	if err := json.Unmarshal([]byte(value.PayloadJSON), &payload); err != nil {
		return job.HandlerFailure{Err: fmt.Errorf("decode cleanup job: %w", err), Terminal: true}
	}
	if payload.Limit <= 0 || payload.Limit > 1000 {
		return job.HandlerFailure{Err: fault.Validation("limit", "cleanup limit must be between 1 and 1000"), Terminal: true}
	}
	now := h.clock.Now()
