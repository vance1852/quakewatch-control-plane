...
func (r *Runner) runDelivery(parent context.Context, value alert.Delivery) {
	...
	if failErr := r.store.FailDelivery(parent, value.ID, r.owner, value.Version, terminal, retryAt, truncate(err.Error(), 1000)); failErr != nil {
...
func permanentMarker(err error) (PermanentError, bool) {
	value, ok := err.(PermanentError)
	return value, ok
}
