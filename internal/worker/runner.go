...
func (r *Runner) runJob(parent context.Context, value job.Job) {
	...
	if failErr := r.store.FailJob(parent, value.ID, r.owner, value.Version, terminal, retryAt, job.ErrorSummary(err.Error(), 1000)); failErr != nil {
...
func (r *Runner) runDelivery(parent context.Context, value alert.Delivery) {
	...
	terminal := value.AttemptCount >= 6 || isPermanent(err)
	retryAt := now.Add(alert.Backoff(value.AttemptCount))
	if failErr := r.store.FailDelivery(parent, value.ID, r.owner, value.Version, terminal, retryAt, job.ErrorSummary(err.Error(), 1000)); failErr != nil {
		r.logger.Error("record alert delivery failure", "delivery_id", value.ID, "error", failErr, "original_error", err)
		return
	}
	r.logger.Warn("alert delivery failed", "delivery_id", value.ID, "attempt", value.AttemptCount, "terminal", terminal, "error", err)
}
